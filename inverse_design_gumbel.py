...
def _build_gumbel_vocabs(opt_cols: list, tech: str) -> dict:
    ...
    for col in opt_cols:
        if col == "capacity_kb":
            ...
            # Fallback: binary {0,1} vocabulary; should not be reached for known columns.
...
class InverseOptimizerGumbel:
    ...
    def optimize(
        ...
        vocabs = {k: v.to(self.device) for k, v in _build_gumbel_vocabs(self.opt_cols, self.tech).items()}
        ...
        for restart in range(max(1, n_restarts)):
            ...
            inner_opt = torch.optim.Adam(list(logits_dict.values()), lr=0.05)
